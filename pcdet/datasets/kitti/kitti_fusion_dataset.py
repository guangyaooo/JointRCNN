...
class KittiFusionDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None,split=None, logger=None):
        """
        Args:
            root_path:
            dataset_cfg:
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if len(annotations['bbox']) >0 and annotations['bbox'][0,0] == -1:
                    annotations['bbox'] = box_utils.lidar_box_to_image_box(gt_boxes_lidar, calib)[0]
    ...
    def __getitem__(self, index):
        ...
        if self.fake_boxes is not None:
            ...
            harmonic_mean = 2 * box_dict['pred_scores'] * box_dict[
                'pred_scores2d'] / (box_dict['pred_scores'] + box_dict[
                'pred_scores2d'] + 1e-8)
    ...
    def prepare_data(self, data_dict):
        ...
        if 'gt_classes' in data_dict:
            gt_boxes = np.concatenate((data_dict['gt_boxes'],
                                       data_dict['gt_classes'].reshape(-1, 1).astype(
                                           np.float32)), axis=1)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = KittiFusionDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    ...
    kitti_infos_train = dataset.get_infos(num_workers=workers,
                                          has_label=dataset_cfg.get('HAS_LABEL',
                                                                    True),

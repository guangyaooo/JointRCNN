...
class RPN(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret = {"in_features": in_features, "min_box_size": cfg.MIN_BOX_SIZE,
               "nms_thresh": cfg.NMS_THRESH,
               "batch_size_per_image": cfg.BATCH_SIZE_PER_IMAGE,
               "positive_fraction": cfg.POSITIVE_FRACTION, "loss_weight": {
               ...
               "post_nms_topk": (
               cfg.POST_NMS_TOPK_TRAIN, cfg.POST_NMS_TOPK_TEST),
               "anchor_generator": build_anchor_generator(cfg,
                                                          [input_shape[f] for f
                                                           in in_features]),
               "anchor_matcher": Matcher(
                   cfg.IOU_THRESHOLDS, cfg.IOU_LABELS,
                   allow_low_quality_matches=True
               ), "head": build_rpn_head(cfg,
    ...
    def label_and_sample_anchors(
        ...
        gt_labels = []
        matched_gt_boxes = []
        for image_size_i, gt_boxes_i, gt_classes_i in zip(image_sizes,
                                                          gt_boxes,
                                                          gt_classes):
            """
